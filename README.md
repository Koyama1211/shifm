...
5. シフト画面で `勤務先マスタ` を選ぶと、勤務先・時給・交通費を自動反映できる。
...
10. 勤務先ごとのルールが必要な場合は `勤務先マスタ管理（低頻度）` で勤務先別の残業/控除ルールを設定する。
11. `月次サマリー` タブで勤務先フィルタを使って月次集計を確認し、勤務先別内訳も確認する。必要ならCSV出力する。
12. Googleカレンダー登録やマスタ保存など低頻度操作は、`シフト` タブの `補助機能（低頻度）` から実行する。
13. 毎週同じ勤務が多い場合は、`シフト` タブの `定型シフト一括登録（週次）` で曜日・期間を指定して一括作成する。
