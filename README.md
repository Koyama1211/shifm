...
- 起動時ログイン認証 (ユーザーID + パスワード)
- 認証復元コードによるログイン情報移行（別ブラウザ/別端末）
- ログイン画面からクラウド認証復元（Token/Gistで復元コードなし移行）
...
- スマホ導線の改善 (日付選択後に `この日の入力へ移動` でフォームへジャンプ)
...
1. `index.html` をブラウザで開き、初回はログイン画面でユーザーID/パスワードを登録する。
2. 2回目以降はログイン画面で認証して利用する。
3. 上部タブで `設定` を開き、`勤務先マスタ管理（低頻度）` を開いてよく使う勤務先の時給/交通費を保存する。
4. 上部タブで `シフト` を開き、カレンダーで日付を選択してシフトを保存する。必要なら同日に複数件登録する。
5. シフト画面で `勤務先マスタ` を選ぶと、勤務先を自動反映し、時給・交通費はマスタ履歴から日付に応じて自動適用される。
6. 同じ勤務先でラインと時間帯が決まっている場合は、`設定` タブの `勤務先マスタ管理（低頻度）` → `ライン・シフトパターン設定` で先に登録する。
7. シフト入力時に `ライン` と `シフトパターン` を選ぶと、開始/終了/休憩が自動入力される。
8. その場で `勤務先マスタに保存` を押して新規マスタ登録もできる。
9. `設定` タブの `給与設定` で残業開始時間・残業倍率・控除率を設定する。
...
1. 既にログインできるブラウザでログイン画面の `ログイン情報を別ブラウザへ移行` を開く。
2. `復元コード生成` を押し、生成されたコードを控える。
3. 新しいブラウザで同じ画面を開き、コードを貼り付けて `復元コード適用` を押す。
4. その後、同じユーザーIDとパスワードでログインする。

## 復元コードを使わずにログイン情報を復元

1. ログイン画面の `クラウドからログイン情報を復元` を開く。
2. `Token` と `Gist ID`（必要ならファイル名）を入力する。
3. `クラウドから復元` を押す。
4. 復元後、同じユーザーIDとパスワードでログインする。
